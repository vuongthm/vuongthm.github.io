...
const COPY = {
  en: {
    heading: "Notes",
    description:
      "Knowledge, skills, mental models, and lessons — things I've learned and want to share.",
    searchPlaceholder: "Search notes… or #tag",
    allTags: "All",
  ...
  vi: {
    heading: "Ghi chú",
    description:
      "Kiến thức, kỹ năng, mô hình tư duy và bài học — những thứ tôi học được và muốn chia sẻ lại.",
    searchPlaceholder: "Tìm kiếm ghi chú… hoặc #thẻ",
    allTags: "Tất cả",
    noResults: "Không tìm thấy ghi chú nào.",
  },
}
    ...
    <>
      ...
      <main className="pt-14">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 pt-12 pb-6 sm:pt-16 sm:pb-8">
          <h1 className="font-serif text-3xl sm:text-4xl font-semibold text-foreground text-balance mb-3 text-center sm:text-left">
            {c.heading}
          </h1>
          <p className="text-base text-muted-foreground leading-relaxed max-w-xl text-pretty mb-8 text-center sm:text-left">
            {c.description}
          </p>
              ...
              >
                {suggestions.map((s, i) => (
                  ...
                  >
                    ...
                    {s.startsWith("#") && (
                      <span className="ml-auto text-xs text-muted-foreground shrink-0">tag</span>
                    )}
                  </button>
                ))}
              </div>
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 pb-16">
          {filtered.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-0">
              {filtered.slice(0, 8).map((note, i) => (
                <div key={note.slug} className={cn(i >= 4 ? "hidden sm:block" : "")}>
